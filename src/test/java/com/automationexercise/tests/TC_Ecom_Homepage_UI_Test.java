...
public class TC_Ecom_Homepage_UI_Test extends BaseTest{
	...
	@Test
	public void TC03_verifyHomeButton() throws IOException {
		ExtentTest test = extent.createTest("verify Home button on home page");
		try {
	...
	//TC04. To verify and validate the E-commerece web application home logo is displayed on home page
		...
		public void TC04_verifyHomeButtonlogo() {
			ExtentTest test = extent.createTest("verify Home logo on home page");
			try {
				 if (homepage.isHomeButtonDisplayed()) {
			            test.pass("Home logo is displayed on the homepage");
			            homepage.clickHomeButton();
			            test.pass("Home button clicked successfully");
			        } else {
	...
	public void TC10_verifyVideoTutorials() throws IOException {
		ExtentTest test = extent.createTest("verify and validate video tutorials button clickability");
		try {
	...
	public void TC12_verifyAPIListProduct() throws IOException {
		...
		try {
			...
			test.pass("API list for products is clicked");
		} catch(Exception e) {
			test.fail("API list for products is not clicked").addScreenCaptureFromPath(ScreenshotUtilities.capturescreenshot(driver, "API List for products is not clicked"));
		}
